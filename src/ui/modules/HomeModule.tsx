/**
 * Home — two tabs (the owner's split, 2026-08-30): DETAILS holds the house
 * itself — value, basis, carrying costs; MORTGAGE holds the loan when there
 * is one. One editing surface — both tabs write the same draft, one Save
 * commits both.
 */
...
const HOME_TABS: ReadonlyArray<TabDef<'details' | 'mortgage'>> = [
...
export function HomeModule() {
  const [tab, setTab] = useState<HomeTabId>('details');
  ...
  return (
    ...
    >
      {(draft, doc) => (
        <TabPanel idPrefix="home" tab={tab}>
          {tab === 'details' && (
            <div className="card">
              <div className="row">
                <NumberField
