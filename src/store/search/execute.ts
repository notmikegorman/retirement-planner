...
import type { EvalOutcome, EvalRequest, SimPool } from "./pool";
import {
  BASELINE_ASSIGNMENT,
  balancedSample,
  enumerateAll,
...
/**
 * What the evaluator looks like FROM the executor: counters it reports and the
 * one method it calls. The real CachedEvaluator (./pool.ts) implements this;
 * the executor tests substitute a modelled world through the same shape.
 */
export interface SearchEvaluator {
  evaluations: number;
  cacheHits: number;
  evaluate(req: EvalRequest): Promise<EvalOutcome>;
}

/**
 * THE ENVIRONMENT SEAM — everything about running a search that differs
 * between node and the browser, injected so this orchestration is shared
 * verbatim (a fork here would silently diverge the search economy the
 * dual-stack gate pins byte-for-byte):
 *
 *   - defaultPoolSize: os.cpus() under node, navigator.hardwareConcurrency in
 *     the browser — both shaped min(8, max(2, cores - 2));
 *   - createPool: the worker_threads pool or the Web Worker pool, same
 *     contract (see ./pool.ts);
 *   - createEvaluator: the CachedEvaluator over that pool and the
 *     environment's own persistence (direct file IO under node; the guarded
 *     main-context proxy in the browser).
 *
 * The node face (src/server/search/execute.ts) binds this once and keeps the
 * historical four-argument runSearch signature for its callers and tests.
 */
export interface SearchEnv {
  defaultPoolSize(): number;
  createPool(size: number, init: { profile: Profile; assumptions: Assumptions }): SimPool;
  createEvaluator(pool: SimPool, profile: Profile, assumptions: Assumptions): SearchEvaluator;
}
...
export async function runSearch(
  ...
  const poolSize = budget.workers ?? env.defaultPoolSize();
  const pool = env.createPool(poolSize, { profile, assumptions });
  const evaluator = env.createEvaluator(pool, profile, assumptions);
  ...
  const evaluateBatch = async (
    ...
    const chunk = Math.max(8, poolSize * 4);
