 ...
 *
 * ENVIRONMENT-NEUTRAL since Phase 3 of the browser port: the module is a
 * factory over a DataStore (which carries the FileStore), and the only two
 * environment services it ever used — node:crypto randomness and the wall
 * clock — are the shared web-crypto wrapper and the same injectable `now`
 * parameters as before. The serialized write chain is unchanged, and it still
 * protects exactly one realm; in the browser that realm is the single IO
 * context that owns the folder (see src/ui/io/browserWriterGuard.ts for what
 * keeps it single).
 */
 ...
 *
 * Local, not UTC, and the store may say so: whether the writer is the
 * localhost server or the browser tab itself, the process computing this and
 * the person reading the page share one machine and one zone, and every date
 * on screen is already formatted in it (netWorthChart.formatSnapshotDate
 * passes no timeZone). A UTC key would put the user in Eastern time into
 * "tomorrow" from 8pm: an evening session would file a second day-start entry
 * mid-sitting, and the row would render a date one day ahead of the guard
 * that wrote it.
 */
...
export interface PlanHistoryStore {
  listPlanHistory(): Promise<PlanHistoryEntry[]>;
  getPlanHistoryEntry(id: string): Promise<PlanHistoryEntry>;
  recordDayStart(plan: Scenario, now: Date): Promise<PlanHistoryEntry | null>;
  keepPlan(plan: Scenario, label?: string, now?: Date): Promise<PlanHistoryEntry>;
...
export function createPlanHistoryStore(data: DataStore): PlanHistoryStore {
  const historyPath = (): string => 'plan-history.json';

  /**
   * Every write to plan-history.json goes through here, one at a time.
   *
   * The file is read-modify-written whole, and it has writers that overlap:
   * the daily guard on the plan's save path, an explicit keep, and a score
   * arriving from a simulation that started minutes earlier. Interleave them
   * and the loser's work is gone — and what would be lost is a version of the
   * plan that no longer exists anywhere else. The guard's check ("does today
   * already have a restore point?") lives INSIDE the same closure as the
   * append for the same reason: the check contains an await, so two
   * concurrent first-edits could both read "no entry today" and both file one.
   *
  ...
  async function readEntries(): Promise<PlanHistoryEntry[]> {
    ...
    try {
      parsed = parseOrThrow(
        ...
        raw,
        `plan history (${data.describeDataFile(filePath)})`,
      );
    } catch (err) {
  ...
  function newEntry(input: {
    ...
    return {
      id: `ph-${input.takenAt.getTime().toString(36)}-${randomHex(3)}`,
  ...
  return {
    /**
     * The history as the page reads it: NEWEST FIRST. The most recent version
     * of the plan is the one most likely to be restored, so it is the one
     * that does not need scrolling to.
     */
    async listPlanHistory(): Promise<PlanHistoryEntry[]> {
      return (await readEntries()).reverse();
    },

    /** One entry by id. Unknown ids are a 404, not an empty answer. */
    async getPlanHistoryEntry(id: string): Promise<PlanHistoryEntry> {
     ...
     * written. Files it if today has no restore point yet; returns null if it
     * has.
     *
     * Only a 'day-start' entry counts. A 'kept' entry is an explicit act — a
     * search finalist put somewhere safe — and letting one satisfy the day
     * would mean that keeping a finalist in the morning silently threw away
     * the undo for everything edited afterwards.
     */
    async recordDayStart(plan: Scenario, now: Date): Promise<PlanHistoryEntry | null> {
     ...
     *
     * This is where a search finalist goes: a twenty-minute search produces
     * six plans worth remembering, and only one of them can be the plan.
     * Keeping one never touches plan.json, so the workbench is exactly where
     * it was.
     */
    async keepPlan(
      plan: Scenario,
      label?: string,
      // The same injectable clock recordDayStart and restorePlan take, and for
      // the same reason: a test that has to file a 'kept' entry alongside a
      // pinned day-start cannot pin the order otherwise. It read the real
      // clock until the afternoon of 2026-08-20, when planStore's "a kept plan
      // does not stand in for the day's restore point" flipped from
      // ['day-start','kept'] to the reverse — a test that had been passing
      // only because it was written before 8:30am.
      now: Date = new Date(),
    ...
    /**
     * Attach the outcome of a scoring attempt to one entry: a score, or the
     * reason there is none. The only edit an entry invites, and it touches
     * nothing but the two score fields — `plan`, `takenAt` and `planHash` are
     * what the entry IS.
     *
     * A RECORDED SCORE IS FINAL. This writes into a blank and never over a
     * number: an entry that already carries a score is refused, and the caller
     * is told which of the two "nothing happened" answers it got. An entry IS
     * a record of a moment — this plan, measured on that day against that
     * day's balances and prices — so a second number filed on the same row
     * would make one row report two different moments as if they were one.
     * (The user's words for the button that used to do it: "Score it again
     * undermines take a snapshot".)
     *
     * A FAILURE IS STILL A BLANK, so `scoreError` may be replaced: "we tried
     * and it failed" records no measurement, and the entry has never been
     * measured. A success arriving over one CLEARS it, because a version
     * carrying both a number and a complaint about not having one gives the
     * reader no way to tell which is current.
     */
    async attachPlanHistoryScore(
      id: string,
      outcome: { score: PlanScore } | { error: string },
    ): Promise<PlanHistoryScoreWrite> {
      return serialized(async () => {
     ...
     * networthStore.attachSustainableSpend gives: the success number is one
     * run and the spend is a bisection of a dozen, so holding the cheap half
     * back until the expensive one lands would let one failure lose both. A
     * version with no score has nowhere to put this and gets `false`.
     *
     * A RECORDED FIGURE IS FINAL, same rule as the score above it: a version
     * that already has a spend figure is refused rather than re-solved.
     ...
     *
     * WHAT THIS DELIBERATELY DOES NOT GUARD, and why it does not have to: an
     * EMPTY spend field on an OLD score. From here that is a blank and gets
     * filled — and any version saved before scoring existed is exactly such a
