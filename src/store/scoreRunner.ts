 ...
 * there isn't one. ENVIRONMENT-NEUTRAL since Phase 4 of the browser port —
 * this is the module that lived at src/server/scoreRunner.ts, with the two
 * environment bindings turned into parameters (the DataStore it reads the
 * profile from, and the run manager inside ScoringDeps) and no behavioural
 * change.
 *
 ...
 * process stayed up — a state that claims work is still happening — which is
 * worse than a plain recorded failure that says nothing was measured.
 */
const TIMEOUT_MS = 20 * 60 * 1000;

/** Injectable seams — the environment's real ones by default, fakes in tests. */
...
/** The real deps for an environment: its run manager and its real clock. */
export function realScoringDeps(runManager: RunManager): ScoringDeps {
  return {
    startRun: runManager.startRun,
    getRun: runManager.getRun,
...
/**
 * WHAT THIS PLAN COULD AFFORD, or why there is no answer — the shape of
 * solveSustainableSpend's reply.
 *
