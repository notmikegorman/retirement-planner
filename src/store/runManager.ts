/**
 * Run manager — ENVIRONMENT-NEUTRAL since Phase 4 of the browser port.
 *
 * This is the same module that lived whole at src/server/runManager.ts, with
 * one structural change and no behavioural ones: everything environment-
 * specific arrives as a parameter. The DataStore carries the folder (the
 * runs/ cache reads and writes), and the actual execution of a simulation —
 * a worker_threads spawn under node, one reusable Web Worker in the browser —
 * arrives as a `RunExecutor`. Everything else moved as-is:
 *
 *   - runKeyFor: the ONE cache-key function (see its comment),
 *   - resolveRunInput: RunRequest → the exact SimulationInput a run executes,
 *   - the runs/<runKey>.json disk cache, written BEFORE the in-memory map
 *     flips to done (finishRun awaits the write — the file is the whole truth
 *     about what has been computed),
 *   - the in-memory runs registry and the queued/running/done/error protocol
 *     the UI polls.
 *
 * The registry is per-INSTANCE now rather than per-module, which changes
 * nothing under node (the server composes exactly one instance) and is the
 * point in the browser: the tab's local backend composes one instance over
 * its own folder, and a second folder could never share a registry with it.
 *
...
/**
 * HOW A SIMULATION ACTUALLY RUNS — the one environment-specific seam.
 *
 * The contract is deliberately a promise and a progress callback, nothing
 * more: resolve with the engine's RunResult exactly as execute() returned it,
 * reject with an Error whose MESSAGE is what the run's error field should say
 * (a worker's message+stack string, an exit-code sentence). The manager never
 * sees a Worker of either kind, which is what lets one manager drive both.
 *
 * Overlapping calls are allowed by the contract and the node executor genuinely
 * runs them in parallel (one worker_threads spawn each); the browser executor
 * SERIALIZES them onto its single reusable worker instead. Both are correct:
 * the manager keys runs by content, dedupes identical ones before ever calling
 * this, and reports queued/running per run — nothing observes the difference
 * except wall-clock overlap.
 */
...
export interface RunManagerOptions {
  ...
  /** Where the non-fatal cache-write failure goes (default console.error). */
  onLog?: (message: string) => void;
}

export function createRunManager(opts: RunManagerOptions): RunManager {
  const { data, executor, onLog = (m) => console.error(m) } = opts;
  ...
  async function readCachedResult(runKey: string): Promise<RunResult | null> {
    try {
      const text = await data.files.readText(runFilePath(runKey));
   ...
   *
   * startRun CANNOT DO THIS JOB, and that is the whole reason this exists.
   ...
   * computed.
   */
  async function lookupCachedRun(req: RunRequest): Promise<RunResult | null> {
   ...
   */
  async function startRun(req: RunRequest): Promise<{ runId: string }> {
    ...
    execute(runId, runKey, input);
    return { runId };
  }

  /**
   * Hand one run to the executor and keep the registry honest about it. The
   * status flow is exactly what spawnWorker's message handlers did: queued at
   * hand-off, running on the first progress frame, then done (via finishRun,
   * which awaits the cache write) or error with the executor's message.
   */
  function execute(runId: string, runKey: string, input: SimulationInput): void {
    runs.set(runId, { runId, status: 'queued', progress: 0 });

    executor
      .run(input, (frac, message) => {
        const current = runs.get(runId);
        if (current && (current.status === 'queued' || current.status === 'running')) {
          runs.set(runId, { runId, status: 'running', progress: frac, message });
        }
      })
      .then(
        (result) => finishRun(runId, runKey, result),
        (err: unknown) => {
          runs.set(runId, {
            runId,
            status: 'error',
  ...
  async function finishRun(runId: string, runKey: string, result: RunResult): Promise<void> {
    // The engine is pure/deterministic and must not read the clock; the manager
    ...
    try {
      await data.files.mkdir('runs');
      await data.files.writeText(runFilePath(runKey), `${JSON.stringify(result, null, 2)}\n`);
    } catch (err) {
      // Cache write failure is not fatal — the result is still served from memory.
      onLog(
        `Failed to write run cache ${data.describeDataFile(runFilePath(runKey))}: ${String(err)}`,
      );
    }
   ...
   * cache so completed runs survive restarts of the process (or, in the
   * browser, of the tab).
   */
  async function getRun(runId: string): Promise<RunProgress | null> {
    const current = runs.get(runId);
