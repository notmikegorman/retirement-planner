 ...
 * in this module; only the refresh route and the snapshot route do.
 *
...
import { ValidationError, loadQuotes, saveQuotes } from './dataStore';
 ...
 *
 * PER-SYMBOL FAILURE IS PER-SYMBOL DATA. One delisted ticker, one timeout or
 * one currency surprise must not take down the other nine quotes — the user
 * pressed one button for their whole portfolio, and the honest answer is "eight
 * stored, VXUS timed out", not a batch error that stored nothing. A failed
 * symbol's PREVIOUS stored quote survives untouched: a stale price with an
 * honest asOf label beats no price.
 *
 * NON-USD IS REJECTED, NOT CONVERTED. Every dollar in the model is USD;
 * storing a Toronto listing's CAD price would silently misprice the account
 * by the exchange rate. The rejection names the currency so the fix (use the
 * US listing) is obvious.
 */
...
/**
 * The refresh route's default batch: every symbol any account holds. An empty
