 ...
 * before Fastify is even constructed. It cannot use this door: the file it is
 * fixing is by definition in a shape the current schema rejects, so the copy it
 * would file could not be stored as a history entry (an entry holds a plan the
...
// shared/util — so importing it here costs the server nothing.
...
import { getPlanHistoryEntry, recordDayStart } from './planHistoryStore';
 ...
 *
 * The guard is a read-then-write: it asks what the plan currently is, decides
 * whether today's restore point exists, and only then overwrites. Both steps
 * contain awaits, so two saves landing together could file the same version
 * twice — or, worse, file the SECOND one as "the day's start" and lose the
 * version the day actually began with. A serial chain costs nothing here (a
 * few dozen writes in a long editing session) and makes that impossible rather
 * than unlikely.
 *
 ...
 * default plan for the profile's people is written and returned, so the first
 * launch lands on a working set of knobs. A file that exists but is malformed
 * or schema-invalid is reported with a message naming plan.json — never
 * silently replaced, since that would discard the user's only plan.
 *
 * The seeding write does NOT file a history entry, and must not: there is no
 * previous version to keep, and an entry holding the plan a folder was born
 * with would offer to "restore" the state before the user had a plan at all.
 */
export async function loadPlan(): Promise<Scenario> {
  const existing = await readPlanFile();
  if (existing !== null) return existing;
  const { people } = await loadProfile();
  const seeded = defaultPlanScenario(people);
  await writeJsonPretty(planPath(), seeded);
  return seeded;
}
 ...
 * of their own analysis, and rewriting it is exactly the kind of edit history
 * exists to undo. So the guard fires on any difference in what would be
 * stored, while `planHash` on the entry keeps answering the narrower question
 * about comparability.
 *
 * A write that changes nothing files nothing: the autosave path can fire a PUT
 * that matches disk byte for byte, and a no-op must be free.
 *
 * `now` is the clock seam — tests pin the day rather than faking global time.
 */
export async function savePlan(scenario: Scenario, now: Date = new Date()): Promise<Scenario> {
  ...
  return serialized(async () => {
    ...
    try {
      ...
      // An unreadable or schema-invalid plan.json cannot be filed as a history
      // entry (an entry holds a plan the engine could run), and it must not
      // block the write either: the user is saving a plan they can see, and
      // refusing would leave them unable to replace the broken file at all.
      previous = null;
    }
    if (previous !== null && stableStringify(previous) !== stableStringify(valid)) {
      await recordDayStart(previous, now);
    }
    await writeJsonPretty(planPath(), valid);
    return valid;
  });
}
 ...
 *
 * It is an ordinary save of a plan that happens to have come from history, and
 * that is what makes a restore undoable: the guard files the version being
 * replaced first, so restoring the wrong one costs nothing. Nothing is
 * consumed, removed or reordered — `entry.plan` is copied forward and the
