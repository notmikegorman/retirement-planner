 ...
 *  - PRICES: the snapshot route refreshes quotes for every holdings symbol
 *    first (this is the one moment besides the Refresh button that the server
 *    fetches), then prices accounts through the SAME resolver every run uses.
 *    Each snapshot stores the prices it used, with their asOf moments, so a
 *    row can always say what it knew.
 ...
 *
 * The file is read-modify-written whole, and it now has TWO writers that can
 * overlap: the snapshot button, and a score arriving from a simulation that
 * started minutes earlier. Interleave them and the loser's work is gone — and
 * the row that would be lost is the one thing in this app that cannot be
 * recreated, because it records prices from a moment that has passed. A serial
 * chain costs nothing here (a handful of writes a month) and makes the lost
 * update impossible rather than unlikely.
 *
...
/**
 * All snapshots, oldest first. A missing file is an empty ledger; a malformed
 * one fails loudly with its filename, like every data file here.
 */
 ...
 * (the route owns the network step so THIS stays computable in tests without
 * one); this function prices the profile from whatever is stored now.
 *
 * A holdings symbol with no stored quote is FATAL here, unlike the profile
 * editor: a snapshot silently priced from a stale or absent quote would be a
 * record of nothing, and the ledger is only worth keeping if every row was
 * true when written.
 */
export async function takeSnapshot(input: {
  ...
  const { profile, derived, missing } = await loadResolvedProfile();
  ...
  // Only the prices the derived balances actually used — a symbol quoted but
  // no longer held would stamp the row with a price that priced nothing.
  ...
  const snapshot: NetWorthSnapshot = {
    id: `nw-${Date.now().toString(36)}-${randomBytes(3).toString('hex')}`,
  ...
  return serialized(async () => {
    ...
    all.push(snapshot);
    await writeJsonPretty(networthPath(), all);
    return snapshot;
  });
}
...
export async function deleteSnapshot(id: string): Promise<void> {
  return serialized(async () => {
    ...
    await writeJsonPretty(networthPath(), next);
  });
}
 ...
 *
 * A SECOND WRITE rather than a bigger first one, because the two halves cost
 * different amounts: the success number is one run, the spend is a bisection of
 * a dozen. Holding the cheap half back until the expensive half lands would
 * mean a crash, a restart or a twenty-minute wedge between them loses both —
 * and the success number is the one the chart is drawn from.
 *
 * A row with no score has nowhere to put this: the score it belonged to is
 * gone (a deleted row), and a spend figure with no probability beside it says
 * nothing on its own. `false` says so without throwing, because this lands
 ...
 *
 * WHAT IT DELIBERATELY DOES NOT GUARD, and why it does not have to: an EMPTY
 * spend field on an OLD score. From here that is a blank and would be filled —
 * and any pre-scoring row is exactly such a blank, one that must never be
 * filled now, because a figure solved against today's balances beside a
 * probability solved months ago makes one row report two moments as one. Reachability is what protects it: `scoreSnapshot` is the only caller and
 * reaches this line only after `attachScore` returned 'attached', which a row
 * with a score never does. So the score under any figure this writes is one the
 * same run computed seconds earlier.
 */
export async function attachSustainableSpend(
  id: string,
